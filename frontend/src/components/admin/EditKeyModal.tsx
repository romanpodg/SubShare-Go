...
export function EditKeyModal({ keyData, onClose, onRefresh }: Props) {
  ...
  const [category, setCategory] = useState(keyData.category || "");
  ...
  useEffect(() => {
    ...
    setCategory(keyData.category || "");
    setKind(keyData.kind);
    setTemplateText(keyData.template_text ?? "");
    setRawConfig(keyData.url);
    setConfigDraft(null);
    setConfigError("");
    setXrayDraft(null);
    ...
    if (isXrayJSONConfiguration(keyData.url)) {
      setConfigMode("xray-json");
      try {
        const parsed = parseXrayJSONConfiguration(keyData.url);
  ...
  const syncLinkConfiguration = (nextRawConfig: string) => {
  ...
  const syncXrayConfiguration = (nextRawConfig: string) => {
    setRawConfig(nextRawConfig);
  ...
  const syncConfiguration = (nextRawConfig: string) => {
    if (configMode === "xray-json") {
      syncXrayConfiguration(nextRawConfig);
      return;
    }
    syncLinkConfiguration(nextRawConfig);
  };
