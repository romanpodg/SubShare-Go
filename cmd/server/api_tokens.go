...
func (a *App) apiV1ListAPITokens(w http.ResponseWriter, r *http.Request) {
	...
	`)
	if err != nil {
		writeV1Error(w, r, http.StatusInternalServerError, "tokens_list_failed", "failed to load API tokens")
		return
	}
	defer rows.Close()
...
func (a *App) apiV1CreateAPIToken(w http.ResponseWriter, r *http.Request) {
	...
	if session.AuthKind != "session" {
		writeV1Error(w, r, http.StatusForbidden, "interactive_session_required", "API tokens can only be created from an interactive session")
		return
	}
	...
	if input.Name == "" || len([]rune(input.Name)) > 80 {
		writeV1Error(w, r, http.StatusBadRequest, "token_name_invalid", "name must contain 1..80 characters")
		return
	}
	scopes, ok := normalizeAPITokenScopes(input.Scopes)
	if !ok {
		writeV1Error(w, r, http.StatusBadRequest, "token_scopes_invalid", "one or more token scopes are invalid")
		return
	}
	...
	if strings.TrimSpace(input.ExpiresAt) != "" {
		...
		if err != nil || !value.After(time.Now()) {
			writeV1Error(w, r, http.StatusBadRequest, "token_expiry_invalid", "expires_at must be a future RFC3339 timestamp")
			return
		}
	...
	if err != nil {
		writeV1Error(w, r, http.StatusInternalServerError, "token_generate_failed", "failed to generate API token")
		return
	}
	raw := "ss_" + random
	...
	if err != nil {
		writeV1Error(w, r, http.StatusInternalServerError, "token_create_failed", "failed to create API token")
		return
	}
...
func (a *App) apiV1RevokeAPIToken(w http.ResponseWriter, r *http.Request) {
	session, _, _ := a.adminSessionFromRequest(r)
	if session.AuthKind != "session" {
		writeV1Error(w, r, http.StatusForbidden, "interactive_session_required", "API tokens can only be revoked from an interactive session")
		return
	}
	id, ok := pathID(w, r, "id")
	if !ok {
	...
	if err != nil {
		writeV1Error(w, r, http.StatusInternalServerError, "token_revoke_failed", "failed to revoke API token")
		return
	}
	if affected, _ := result.RowsAffected(); affected == 0 {
		writeV1Error(w, r, http.StatusNotFound, "token_not_found", "active API token not found")
		return
	}
	a.recordAuditEvent(r, "api_token.revoke", "api_token", strconv.FormatInt(id, 10), nil)
	writeMessage(w, "API token revoked")
}
