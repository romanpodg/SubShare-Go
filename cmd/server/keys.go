...
func (a *App) registerKeyRoutes(mux *http.ServeMux) {
	...
	mux.Handle("DELETE /api/v1/keys/{id}", a.requireAdmin(httpapi.V1Envelope(http.HandlerFunc(k.legacy.DeleteKey))))
	mux.Handle("POST /api/v1/keys/{id}/check", a.requireAdmin(httpapi.V1Envelope(http.HandlerFunc(k.admin.CheckKey))))
